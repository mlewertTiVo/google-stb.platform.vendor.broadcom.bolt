...
#if defined(BCHP_DVP_HR_REG_START) && defined(BCHP_HDMI_RX_FE_0_REG_START)
...
#define RDB_F_DEF(reg, field)\
		(BCHP_##reg##_##field##_DEFAULT << BCHP_##reg##_##field##_SHIFT)

#define RDB_F_NUM(reg, field, num)\
		(num << BCHP_##reg##_##field##_SHIFT)
...
static void __maybe_unused bcm7364_new_defaults(void)
{
	BDEV_WR(BCHP_CLKGEN_PLL_LC_PLL_DIV,
		RDB_F_NUM(CLKGEN_PLL_LC_PLL_DIV, PDIV, 1) |
		RDB_F_NUM(CLKGEN_PLL_LC_PLL_DIV, NDIV_INT, 60));
	/*
	 * The default is for no hold, phase offset or disable on
	 * all the PLL_LC channels, hence we write instead of
	 * read-modify-write.
	 *
	 *              MDIV_CH<n> = new value
	 * POST_DIVIDER_HOLD_CH<n> = 0 (default)
	 *      PHASE_OFFSET_CH<n> = 0 (default)
	 *         CLOCK_DIS_CH<n> = 0 (default)
	 *
	 * NDIV = 60
	 * NDIV_INT = 60
	 * NDIV_FRAC = 0
	 ...
	 * Ch5 = 33
	 *
	 * CH0 frequency = 60 MHz      USB2
	 * CH1 frequency = 54 MHz      USB3
	 * CH2 frequency = 54 MHz      GENET
	 * CH3 frequency = 30 MHz      USB HUB CLK TO PAD
	 * CH4 frequency = 108 MHz     FSK CLOCK
	 * CH5 frequency = 98.18 MHz   SDIO CLK
	 */
	BDEV_WR(BCHP_CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_0,
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_0,
			POST_DIVIDER_HOLD_CH0) |
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_0,
			PHASE_OFFSET_CH0) |
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_0, CLOCK_DIS_CH0) |
		RDB_F_NUM(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_0, MDIV_CH0, 54));

	BDEV_WR(BCHP_CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_1,
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_1,
			POST_DIVIDER_HOLD_CH1) |
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_1,
			PHASE_OFFSET_CH1) |
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_1, CLOCK_DIS_CH1) |
		RDB_F_NUM(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_1, MDIV_CH1, 60));

	BDEV_WR(BCHP_CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_2,
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_2,
			POST_DIVIDER_HOLD_CH2) |
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_2,
			PHASE_OFFSET_CH2) |
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_2, CLOCK_DIS_CH2) |
		RDB_F_NUM(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_2, MDIV_CH2, 60));

	BDEV_WR(BCHP_CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_3,
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_3,
			POST_DIVIDER_HOLD_CH3) |
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_3,
			PHASE_OFFSET_CH3) |
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_3, CLOCK_DIS_CH3) |
		RDB_F_NUM(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_3, MDIV_CH3, 108));

	BDEV_WR(BCHP_CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_4,
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_4,
			POST_DIVIDER_HOLD_CH4) |
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_4,
			PHASE_OFFSET_CH4) |
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_4, CLOCK_DIS_CH4) |
		RDB_F_NUM(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_4, MDIV_CH4, 30));

	BDEV_WR(BCHP_CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_5,
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_5,
			POST_DIVIDER_HOLD_CH5) |
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_5,
			PHASE_OFFSET_CH5) |
		RDB_F_DEF(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_5, CLOCK_DIS_CH5) |
		RDB_F_NUM(CLKGEN_PLL_LC_PLL_CHANNEL_CTRL_CH_5, MDIV_CH5, 33));
}
...
#endif /* BCHP_DVP_HR_REG_START && BCHP_HDMI_RX_FE_0_REG_START */
}
...
#if BCHP_CLKGEN_PLL_CPU_DVFS_SM_ARRAY_FRAC_ARRAY_UPPER_INDEX_DEFAULT != 2
	#error "DVFS NDIV State Machine requires adjustment INDEX!"
#endif
#if BCHP_CLKGEN_PLL_CPU_DVFS_SM_ARRAY_FRAC_ARRAY0_DEFAULT != 0
	#error "DVFS NDIV State Machine requires adjustment ARRAY0!"
#endif
#if BCHP_CLKGEN_PLL_CPU_DVFS_SM_ARRAY_FRAC_ARRAY1_DEFAULT != 2
	#error "DVFS NDIV State Machine requires adjustment ARRAY1!"
#endif
#if BCHP_CLKGEN_PLL_CPU_DVFS_SM_ARRAY_FRAC_ARRAY2_DEFAULT != 5
	#error "DVFS NDIV State Machine requires adjustment ARRAY2!"
#endif

#define MAX_MDIV (BCHP_CLKGEN_PLL_SYS0_PLL_CHANNEL_CTRL_CH_2_MDIV_CH2_MASK >> \
		BCHP_CLKGEN_PLL_SYS0_PLL_CHANNEL_CTRL_CH_2_MDIV_CH2_SHIFT)
#define MAX_NDIV (BCHP_CLKGEN_PLL_CPU_PLL_DIV_4K_NDIV_INT_MASK >> \
		BCHP_CLKGEN_PLL_CPU_PLL_DIV_4K_NDIV_INT_SHIFT)

static void run_dvfs_statemachine(unsigned int ndiv_new)
{
	uint32_t ndiv_current;
	uint32_t status;
	uint32_t control0;

	if (ndiv_new < MIN_NDIV_DVFS_STATE_MACHINE || ndiv_new >= MAX_NDIV)
		return;

	/* get the current NDIV */
	ndiv_current = BDEV_RD(BCHP_CLKGEN_PLL_CPU_PLL_DIV_4K);
	ndiv_current &= BCHP_CLKGEN_PLL_CPU_PLL_DIV_4K_NDIV_INT_MASK;
	ndiv_current >>= BCHP_CLKGEN_PLL_CPU_PLL_DIV_4K_NDIV_INT_SHIFT;
	if (ndiv_new == ndiv_current)
		return;

	/* stop DVFS NDIV state machine */
	BDEV_WR(BCHP_CLKGEN_PLL_CPU_DVFS_SM_CONTROL0,
		BCHP_CLKGEN_PLL_CPU_DVFS_SM_CONTROL0_GOTO_IDLE_MASK);
	dsb();
	/* wait until SM stops */
	status = BDEV_RD(BCHP_CLKGEN_PLL_CPU_DVFS_SM_STATUS);
	while (status & BCHP_CLKGEN_PLL_CPU_DVFS_SM_STATUS_CURRENT_STATE_MASK)
		status = BDEV_RD(BCHP_CLKGEN_PLL_CPU_DVFS_SM_STATUS);
	/* load fractional array: no need as POR values have been verified */
	/* set NDIV target and timer (default: 0x438) */
	BDEV_WR(BCHP_CLKGEN_PLL_CPU_DVFS_SM_CONTROL1,
		RDB_F_NUM(CLKGEN_PLL_CPU_DVFS_SM_CONTROL1,
			TIMER_VALUE, 0x438) |
		RDB_F_NUM(CLKGEN_PLL_CPU_DVFS_SM_CONTROL1,
			NDIV_TARGET, ndiv_new));
	dsb();

	control0 = BCHP_CLKGEN_PLL_CPU_DVFS_SM_CONTROL0_USE_NDIV_FROM_SM_MASK;
	if (ndiv_new > ndiv_current)
		control0 |= BCHP_CLKGEN_PLL_CPU_DVFS_SM_CONTROL0_INCREMENT_MASK;
	control0 |= BCHP_CLKGEN_PLL_CPU_DVFS_SM_CONTROL0_GOTO_IDLE_MASK;
	/* fill up control values before starting SM */
	BDEV_WR(BCHP_CLKGEN_PLL_CPU_DVFS_SM_CONTROL0, control0);
	dsb();
	/* now, give control to SM */
	control0 &= ~BCHP_CLKGEN_PLL_CPU_DVFS_SM_CONTROL0_GOTO_IDLE_MASK;
	BDEV_WR(BCHP_CLKGEN_PLL_CPU_DVFS_SM_CONTROL0, control0);
	/* update RDB NDIV to SM NDIV to avoid confusion, especially when
	 * SM gets stopped.
	 */
	BDEV_WR(BCHP_CLKGEN_PLL_CPU_PLL_DIV_4K,
		ndiv_new << BCHP_CLKGEN_PLL_CPU_PLL_DIV_4K_NDIV_INT_SHIFT);
	dsb();
	/* wait until SM stops */
	status = BDEV_RD(BCHP_CLKGEN_PLL_CPU_DVFS_SM_STATUS);
	while (status & BCHP_CLKGEN_PLL_CPU_DVFS_SM_STATUS_CURRENT_STATE_MASK)
	...
	struct clock_divisors scb_clks)
{
	...
	if (cpu_clks.mdiv > 0)
		BDEV_WR(BCHP_CLKGEN_PLL_CPU_PLL_CHANNEL_CTRL_CH_0_4K,
			RDB_F_NUM(CLKGEN_PLL_CPU_PLL_CHANNEL_CTRL_CH_0_4K,
				MDIV_CH0, cpu_clks.mdiv));
	dsb();

	run_dvfs_statemachine(cpu_clks.ndiv);
}
#endif
...
#if defined(BCHP_SUN_TOP_CTRL_GENERAL_CTRL_0_clk_freerun_mode_DEFAULT)
#if BCHP_SUN_TOP_CTRL_GENERAL_CTRL_0_clk_freerun_mode_DEFAULT == 1
	/* enable CBR clock gating iff disabled via POR */
	BDEV_WR_F(SUN_TOP_CTRL_GENERAL_CTRL_0, clk_freerun_mode, 0);
#endif
#endif

#if defined(CONFIG_BCM7364)
#if defined(CONFIG_BCM7364C0)
	bcm7364_new_defaults();
#endif
