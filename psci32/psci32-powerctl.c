...
static inline void reg_bit_set(uint32_t reg, uint32_t mask)
{
...
static inline void reg_bit_clr(uint32_t reg, uint32_t mask)
{
	rdb_write(reg, rdb_read(reg) & (~mask));
	BARRIER();
}
...
static int wait_bit_set(uint32_t reg, uint32_t mask)
{
...
static int wait_bit_clr(uint32_t reg, uint32_t mask)
{
	int to = BPCM_TIMEOUT_US / BPCM_WAIT_US;
