...
SAVED_FAMILY := $(shell [ -e .family ] && cat .family || echo unknown)
SAVED_CFG := $(shell [ -e .config ] && cat .config || echo unknown)
...
$(CFG_GEN_FILES) .family: $(CFG_GEN_FILES_DEPS)
	$(Q)echo "  ================================ "
	$(Q)echo "  CFG.PL  $(CFG)"
	$(Q)echo "  FAMILY  $(FAMILY)"
	$(Q)echo "  ================================"
	$(Q)$(SCRIPT_ENG) -f $(FAMILY) -c $(CFG) -r include/$(FAMILY) -D $(ODIR) $(CFG_SINGLE_BOARD)
	$(Q)cp gen/$(FAMILY)/family .family
	$(Q)echo -n $(CFG) > .config
...
env: dtc/.marker zlib-$(ZLIB_VER)/.marker scripts/mcb2c scripts/splash_create_flash_file $(ROOT)/scripts/patcher.pl

zlib_check = $(shell scripts/check_for_zlib $(SYSCC) $(SYSZLIB) 1>&2 2> /dev/null; echo $$?)

zlib-$(ZLIB_VER)/.marker: thirdparty/zlib-$(ZLIB_VER).tar.gz
	$(Q)echo "         SETUP ZLIB"
	$(Q)tar -xzf thirdparty/zlib-$(ZLIB_VER).tar.gz
	$(Q)patch -p0 < thirdparty/zlib-$(ZLIB_VER).patch
	$(Q)touch $@

dtc/.marker: thirdparty/GPL-BSD-dtc-$(DTC_COMMIT).tgz \
		thirdparty/libfdt_env.patch
	$(Q)echo "         SETUP DTC"
	$(Q)tar -xzf thirdparty/GPL-BSD-dtc-$(DTC_COMMIT).tgz
	$(Q)CROSS_COMPILE= $(MAKE) -C dtc dtc
	$(Q)patch -p0 < thirdparty/libfdt_env.patch
	$(Q)patch -p0 < thirdparty/libfdt_coverity.patch
	$(Q)$(MAKE) -C dtc libfdt_clean
	$(Q)touch $@

scripts/mcb2c: scripts/mcb2c.c
	$(Q)echo "         SETUP MCB COMPACT"
	$(Q)$(SYSCC) $(SYSCFLAGS) scripts/mcb2c.c -o scripts/mcb2c

scripts/splash_create_flash_file: scripts/splash_create_flash_file.c
...
distclean:
	...
	$(Q)rm -f .family toolpath .config
	$(Q)rm -f $(CFG_GEN_FILES)
	$(Q)rm -rf gen # legacy
	$(Q)rm -rf build/[0-9]* # legacy
	$(Q)rm -f scripts/mcb2c
	$(Q)rm -f scripts/splash_create_flash_file
	$(Q)rm -f scripts/patcher.pl
	$(Q)rm -rf bolts objs
	$(Q)rm -rf zlib-$(ZLIB_VER)
	$(Q)rm -f doc/*.html
	$(Q)rm -f ./.doxyfile
	$(Q)rm -fr ./doxy
