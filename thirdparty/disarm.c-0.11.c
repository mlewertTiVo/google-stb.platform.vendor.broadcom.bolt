...
/*
  All modifications (point #2) are bounded
 by "_BOLT_" preprocessor selections.
 
 * Basic ARM disassembly
 * On ARMv7 some instructions are not recognised.
...
void disasm_inst(char *buf, int buf_size, uint32_t inst, uint64_t pc) {
	...
	instr = instr_disassemble(w, addr, &options);
	buf += xsprintf(buf, "%s", instr->text);
	if (instr->undefined || instr->badbits || instr->oddbits) {
		buf += xsprintf(buf,"  ;");
		if (instr->undefined) 
			buf += xsprintf(buf," [undefined instr]");
		if (instr->badbits) 
			buf += xsprintf(buf," [illegal bits]");
		if (instr->oddbits) 
			buf += xsprintf(buf," [unexpected bits]");
	}
}
#else /* original code */
static void disassemble(const char * filename, unsigned long base) {
  ...
  while (fread(&w, 4, 1, f) != 0) {
    ...
    if (instr->undefined || instr->badbits || instr->oddbits) {
      printf("  ;");
      if (instr->undefined) printf(" [undefined instr]");
      if (instr->badbits) printf(" [illegal bits]");
      if (instr->oddbits) printf(" [unexpected bits]");
    }
    printf("\n");
...
#ifndef _BOLT_
extern int main(int ac, const char * av[]) {
  ...
  while (ac >= 2 && av[1][0]=='-') {
    if (!strcmp(av[1], "-r")) {
      options.flags |= disopt_ReverseBytes;
    }
    else if (!strcmp(av[1], "-v")) {
      fprintf(stderr, "[This is disarm, version 0.10]\n");
    }
    else if (!strcmp(av[1], "--")) {
      --ac; ++av; break;
    }
    else {
      fprintf(stderr, "[I don't understand the %s option.]\n", av[1]);
    }
    --ac; ++av;
  }

  if (ac != 3) {
    fprintf(stderr, "Usage: disarm [-rv] filename base\n");
    return 1;
  }

  filename = av[1];
  base     = strtol(av[2], 0, 0);

  disassemble(filename, base);
