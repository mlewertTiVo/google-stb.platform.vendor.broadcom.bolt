...
static int _udp_rx_callback(void *ref, ebuf_t *buf, uint8_t *destaddr,
			    uint8_t *srcaddr);
...
int _udp_socket(udp_info_t *info, uint16_t port)
{
	int idx;
	udp_port_t *udp;
	uint16_t srcport = UDP_PORTBASE + (arch_getticks() & 0xFFF);

	while (_udp_find_port(info, srcport)) {	/* should always exit */
		srcport++;
		if (srcport > (UDP_PORTBASE + 4096))
			srcport = UDP_PORTBASE;
	}
...
int _udp_bind(udp_info_t *info, int s, uint16_t port)
{
	...
	if (_udp_find_port(info, port))
		return BOLT_ERR_ALREADYBOUND;
