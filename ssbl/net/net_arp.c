...
static int arp_rx_callback(ebuf_t *buf, void *ref)
{
	...
	if (t16 != ARP_HWADDRSPACE_ETHERNET)
		goto drop;
	...
	if (memcmp(targetip, ipi->net_info.ip_addr, IP_ADDR_LEN) != 0)
		goto drop;
...
void _arp_timer_tick(ip_info_t *ipi)
{
	...
	for (idx = 0; idx < ARP_TABLE_SIZE; idx++, ae++) {
		...
		case ae_arping:
			 ...
			 * and retransmit the ARP message.
			 */
			ae->ae_timer--;
			if (ae->ae_timer <= 0) {
				if (ae->ae_retries == 0) {
...
int _arp_init(ip_info_t *ipi)
{
	...
	ipi->arp_port =
	    eth_open(ipi->eth_info, ETH_PTYPE_DIX, (char *)arpproto,
		     arp_rx_callback, ipi);
