...
ebuf_t *udp_recv_with_timeout(int s, int seconds)
{
	...
	TIMER_SET(timer, seconds * BOLT_HZ);

	while (!TIMER_EXPIRED(timer)) {
		POLL();
		/* our net context (device) went away while
		 * we were waiting.
		 */
		if (!netctx)
			return NULL;
...
int tcp_recv(int s, uint8_t * buf, int len)
{
	...
	while (len > 0) {
		...
		if (res == 0) {
			_tcp_status(netctx->tcpinfo, s, &connflag, NULL, NULL);
			if (connflag != TCPSTATUS_CONNECTED) {
				res = BOLT_ERR_NOTCONN;
				break;
			}
		}
...
{
	if (netctx)
		_arp_add(netctx->ipinfo, destip, desthw);
}
...
uint8_t *net_getparam(int param)
{
	if (!netctx)
		return NULL;
	if (param == NET_DEVNAME)
		return (uint8_t *) netctx->devname;
...
static void net_poll(void *arg)
{
	if (netctx) {
		...
		if (TIMER_EXPIRED(netctx->timer)) {
			...
			TIMER_SET(netctx->timer, BOLT_HZ);
		}
	}
}
...
/**********************************************************************
  ...
  *
  *  Input parameters:
  *	devname - BOLT device name for network device
  *
  *  Return value:
  *	0 if ok
  *	else error code
...
int net_init(char *devname)
{
	...
	bolt_bg_add(net_poll, ctx);
	TIMER_SET(ctx->timer, BOLT_HZ);

#if CFG_TCP
	ctx->tcpinfo = _tcp_init(ctx->ipinfo, ctx->ipinfo);
	bolt_bg_add(_tcp_poll, ctx->tcpinfo);
#endif
...
/**********************************************************************
  *  net_unregister(devname)
  *
  *  Unregister a device from the network stack.
  *
  *  Input parameters:
  *	devname - BOLT device name for network device
  *
...
{
	int rc = BOLT_ERR_DEVNOTFOUND;

	if (!netctx)
		return BOLT_ERR_NETDOWN;

	if (netctx->devname && !strcmp(netctx->devname, devname)) {
		net_uninit();
		net_setnetvars();
		rc = BOLT_OK;
	}
...
void net_setnetvars(void)
{
	...
	x = (char *)net_getparam(NET_DEVNAME);
	if (x)
		env_setenv("NET_DEVICE", x, ENV_FLG_BUILTIN);

	x = (char *)net_getparam(NET_DOMAIN);
	if (x)
		env_setenv("NET_DOMAIN", x, ENV_FLG_BUILTIN);
