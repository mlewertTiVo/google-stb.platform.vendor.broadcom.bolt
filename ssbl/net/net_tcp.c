...
struct tcp_info_s {
	...
	bolt_timer_t ti_fasttimer;	/* 200ms timer */
...
/*  *********************************************************************
    ...
    ********************************************************************* */
extern bolt_driver_t tcpconsole;
extern bolt_driver_t tcpfastboot;

static void _tcp_protosend(tcp_info_t * ti, tcb_t * tcb);
static int _tcp_rx_callback(void *ref, ebuf_t * buf, uint8_t * destaddr,
			    uint8_t * srcaddr);
...
static void _tcp_sendctlmsg(tcp_info_t * ti, tcb_t * tcb, uint16_t flags,
			    int timeout);
...
/*  *********************************************************************
    *  _tcp_oos_init(tcb_t *tcb)
    *
    *  Helper function to initialize data structure members for handling
...
static void _tcp_oos_init(tcb_t *tcb) {
	int i;

	tcb->tcb_oos = 0;
	tcb->tcb_oos_max = 0;
	tcb->tcb_oos_err_toomany = 0;
	tcb->tcb_oos_err_nomemory = 0;
	tcb->tcb_oos_err_badsync = 0;
	for (i = 0; i < TCP_MAX_OOS_BUFS; i++) {
		tcb->tcb_oos_buffers[i].seqnum = 0;
		tcb->tcb_oos_buffers[i].flags = 0;
		tcb->tcb_oos_buffers[i].datalen = 0;
		if (tcb->tcb_oos_buffers[i].buf) {
			KFREE(tcb->tcb_oos_buffers[i].buf);
			tcb->tcb_oos_buffers[i].buf = NULL;
		}
	}
}
...
tcp_info_t *_tcp_init(ip_info_t * ipi, void *ref)
{
	...
	ti->ti_iss = (uint32_t) arch_getticks();
	...
	/*
	 * Add TCP stack as BOLT device to abstract read/write access from
	 * other modules.
	 * Note that we have to add all the TCP devices here as the TCP
...
static void _tcp_freetcb(tcp_info_t * ti, tcb_t * tcb)
{
	...
	if (tcb->tcb_socknum >= 0)
		ti->ti_ports[tcb->tcb_socknum] = NULL;
	tcb->tcb_socknum = -1;
...
int _tcp_socket(tcp_info_t * info)
{
	...
	if (info->ti_onqueue >= TCP_MAX_TCBS)
	...
	if (tmb_alloc(&tcb->tcb_txbuf, TCP_BUF_SIZE) < 0)
		goto error;
	if (tmb_alloc(&tcb->tcb_rxbuf, TCP_BUF_SIZE) < 0)
		goto error;
	...
	tcb->tcb_lclport = 1024 + (arch_getticks() % (UINT16_MAX - 1024));

	while (_tcp_find_lclport(info, tcb->tcb_lclport) != NULL) {
		tcb->tcb_lclport++;
		if (tcb->tcb_lclport == 0)
			tcb->tcb_lclport = 1024;
	}
...
int _tcp_close(tcp_info_t * ti, int s)
{
	...
	case TCPSTATE_ESTABLISHED:
		...
		_tcp_sendctlmsg(ti, tcb, TCPFLG_ACK | TCPFLG_FIN,
				TCP_RETX_TIMER);
		break;
...
static void _tcp_closetcb(tcp_info_t * ti, tcb_t * tcb)
{
	...
	/*
	 * FIXME - Always report debug info for out-of-order packets
	 * until we submit this patch to mainline BOLT
	 */
	if (tcb->tcb_oos_max) {
		printf("Max OOS = %u\n", tcb->tcb_oos_max);
		printf("OOS toomany  = %u\n", tcb->tcb_oos_err_toomany);
		printf("OOS nomemory = %u\n", tcb->tcb_oos_err_nomemory);
		printf("OOS badsync  = %u\n", tcb->tcb_oos_err_badsync);
	}
	_tcp_oos_init(tcb);
}
...
int _tcp_recv(tcp_info_t * ti, int s, uint8_t * buf, int len)
{
	...
	return tmb_copyout(&(tcb->tcb_rxbuf), buf, len, TRUE);
}
...
int _tcp_bind(tcp_info_t * ti, int s, uint16_t port)
{
	...
	if (_tcp_find_lclport(ti, port))
		return BOLT_ERR_ADDRINUSE;
...
int _tcp_peeraddr(tcp_info_t * ti, int s, uint8_t * addr, uint16_t * port)
{
	...
	if (addr)
		memcpy(addr, tcb->tcb_peeraddr, IP_ADDR_LEN);
	if (port)
		*port = tcb->tcb_peerport;
	...
	if (connflag) {
		if ((tcb->tcb_state == TCPSTATE_ESTABLISHED) ||
			...
			*connflag = TCPSTATUS_CONNECTED;
		    ...
		    if (TCPSTATE_IN_SET
			(tcb->tcb_state, M_TCPSTATE_CONNINPROGRESS)) {
...
int _tcp_debug(tcp_info_t * ti, int s, int arg)
{
	...
	printf("OOS=%d MaxOOS=%u\n",
	       tcb->tcb_oos, tcb->tcb_oos_max);
...
static void _tcp_output(tcp_info_t * ti, tcb_t * tcb)
{
	...
	if (TCPSEQ_GT(sendmax, windowmax))
		sendmax = windowmax;
	if (TCPSEQ_GT(sendmax, cwndmax))
		sendmax = cwndmax;
...
static void _tcp_protosend(tcp_info_t * ti, tcb_t * tcb)
{
	...
	if (flags & (TCPFLG_SYN | TCPFLG_FIN))
		tcb->tcb_sendnext++;
...
static tcb_t *_tcp_find_tcb(tcp_info_t * ti, uint16_t srcport, uint16_t dstport,
			    uint8_t * saddr)
{
	tcb_t *tcb = NULL;
	queue_t *qb;
	...
	for (qb = ti->ti_tcblist.q_next; qb != &(ti->ti_tcblist);
		...
		if ((tcb->tcb_peerport != 0) &&
		    ...
		    (memcmp(saddr, tcb->tcb_peeraddr, IP_ADDR_LEN) == 0))
			break;
	}
	...
	for (qb = ti->ti_tcblist.q_next; qb != &(ti->ti_tcblist);
		...
		/* Try active TCBs */
		if ((tcb->tcb_peerport == 0) && (tcb->tcb_lclport == dstport))
			break;
	}
...
static void _tcp_procack(tcp_info_t * ti, tcb_t * tcb, uint32_t ack,
			 uint16_t flags)
{
	int ackamt;
	int unacklen;
...
/*  *********************************************************************
    *  _tcp_proc_oos(tcb,seqnum,flags,buf,datalen)
    *
    *  Helper function for _tcp_procdata() to handle out-of-sequence/order
    *  packet
    *
    *  Input parameters:
    *  	   tcb - tcb describing TCP session
    *  	   seqnum,flags - from the header of the received TCP packet
    *  	   buf - ebuf containing data
...
static int _tcp_proc_oos(tcb_t * tcb, uint32_t seqnum, uint16_t flags, uint8_t * buf, int datalen)
{
	uint8_t *oos_buf;
	int i;

	if (TCPSEQ_LEQ(seqnum, tcb->tcb_rcvnext))
		return 0;

	if (tcb->tcb_oos >= TCP_MAX_OOS_BUFS) {
		DEBUGMSG(("Too many out-of-order packets\n"));
		tcb->tcb_oos_err_toomany++;
		return 0;
	}

	oos_buf = (uint8_t *)KMALLOC(datalen, 0);
	if (!oos_buf) {
		DEBUGMSG(("Out of memory for out-of-order packets\n"));
		tcb->tcb_oos_err_nomemory++;
		return 0;
	}

	for (i = 0; i < TCP_MAX_OOS_BUFS; i++) {
		if ((tcb->tcb_oos_buffers[i].buf) == NULL)
			break;
	}
	if (i >= TCP_MAX_OOS_BUFS) {
		DEBUGMSG(("Out-of-order tracking out of sync\n"));
		KFREE(oos_buf);
		tcb->tcb_oos_err_badsync++;
		return 0;
	}

	tcb->tcb_oos_buffers[i].seqnum = seqnum;
	tcb->tcb_oos_buffers[i].flags = flags;
	tcb->tcb_oos_buffers[i].datalen = datalen;
	tcb->tcb_oos_buffers[i].buf = oos_buf;
	memcpy(oos_buf, buf, datalen);
	tcb->tcb_oos++;
	if (tcb->tcb_oos > tcb->tcb_oos_max)
		tcb->tcb_oos_max = tcb->tcb_oos;
	DEBUGMSG(("Saved %u %u %u into slot %u/%u/%u\n",
		seqnum, flags, datalen, i, tcb->tcb_oos, tcb->tcb_oos_max));
	return 1;
}
			  ...
			  uint16_t flags, ebuf_t * buf)
{
	...
	if (datalen != 0) {
		...
		if (!rwin) {
			...
			if (_tcp_proc_oos(tcb, seqnum, flags, bp, datalen)) {
				DEBUGMSG(("Saving out-of-order packet %u %u %u\n",
		...
		if (reallen > 0) {
			...
			reallen =
			    tmb_copyin(&(tcb->tcb_rxbuf), bp, reallen, TRUE);

			tcb->tcb_rcvnext += reallen;

			/* Check saved out-of-sequence buffers for more data */
			while (tcb->tcb_oos) {
				int consumed = 0;
				int i;
				for (i = 0; i < TCP_MAX_OOS_BUFS; i++) {
					if (tcb->tcb_oos_buffers[i].buf) {
						if (tcb->tcb_oos_buffers[i].seqnum == tcb->tcb_rcvnext) {
							reallen = tmb_copyin(&(tcb->tcb_rxbuf),
								tcb->tcb_oos_buffers[i].buf,
								tcb->tcb_oos_buffers[i].datalen, TRUE);
							tcb->tcb_rcvnext += reallen;
							DEBUGMSG(("Conusmed %u/%u bytes from out-of-order packet %u at %u\n",
								  reallen, tcb->tcb_oos_buffers[i].datalen,
								  i, tcb->tcb_oos_buffers[i].seqnum));
						} else if (TCPSEQ_LT(tcb->tcb_oos_buffers[i].seqnum, tcb->tcb_rcvnext)) {
							reallen = TCPSEQ_DIFF(tcb->tcb_rcvnext,
									tcb->tcb_oos_buffers[i].seqnum);
							if (reallen > tcb->tcb_oos_buffers[i].datalen)
								reallen = tcb->tcb_oos_buffers[i].datalen;
							DEBUGMSG(("Discarded %u/%u bytes from out-of-order packet %u at %u\n",
								  reallen, tcb->tcb_oos_buffers[i].datalen,
								  i, tcb->tcb_oos_buffers[i].seqnum));
						} else {
							continue;
						...
						if (tcb->tcb_oos_buffers[i].datalen == reallen) {
							tcb->tcb_oos_buffers[i].seqnum = 0;
							tcb->tcb_oos_buffers[i].flags = 0;
							tcb->tcb_oos_buffers[i].datalen = 0;
							KFREE(tcb->tcb_oos_buffers[i].buf);
							tcb->tcb_oos_buffers[i].buf = NULL;
							tcb->tcb_oos--;
							DEBUGMSG(("Freed slot %u/%u\n", i, tcb->tcb_oos));
						} else {
							tcb->tcb_oos_buffers[i].seqnum += reallen;
							tcb->tcb_oos_buffers[i].datalen -= reallen;
							memmove(tcb->tcb_oos_buffers[i].buf,
								tcb->tcb_oos_buffers[i].buf + reallen,
								tcb->tcb_oos_buffers[i].datalen);
							DEBUGMSG(("Adjusted slot %u/%u\n", i, tcb->tcb_oos));
						}
						consumed = 1;
					}
				}
			    ...
			    uint8_t * srcaddr)
{
	...
	if (!tcb) {
		char srcip[20];
		sprintf_ip(srcip, srcaddr);
		DEBUGMSG(("Invalid TCB from %s, srcport=%u dstport=%u\n",
	...
	case TCPSTATE_LISTEN:
		if (flags & TCPFLG_SYN) {
			...
			_tcp_sendctlmsg(ti, tcb, TCPFLG_SYN | TCPFLG_ACK,
					TCP_RETX_TIMER);
		}
	...
	case TCPSTATE_SYN_SENT:
		...
		if (flags & TCPFLG_SYN) {
			...
			if ((flags & TCPFLG_ACK) &&
				...
				_tcp_setstate(tcb, TCPSTATE_SYN_RECEIVED);
				_tcp_sendctlmsg(ti, tcb,
						TCPFLG_SYN | TCPFLG_ACK,
						TCP_RETX_TIMER);
			}
		}
...
/*  *********************************************************************
    *  _tcp_poll(arg)
    *  
    *  BOLT calls this routine periodically to allow us to check
