...
#define DHCP_REQ_TIMEOUT	(3*BOLT_HZ)
#define DHCP_NUM_RETRIES	2 /* (8) 2 mins instead of 8 mins. */
...
#define DHCP_TAG_SCRIPT		130	/* BOLT extended option */
#define DHCP_TAG_OPTIONS	131	/* BOLT extended option */
...
void dhcp_set_envvars(dhcpreply_t *reply)
{
	...
	if (reply->dr_bootserver[0] | reply->dr_bootserver[1] |
		...
		sprintf_ip(buffer, reply->dr_bootserver);
...
static void dhcp_dumptag(uint8_t tag, uint8_t len, uint8_t *buf)
{
	...
	case DHCP_TAG_SCRIPT:
		buf[len] = 0;
		xprintf("BOLT Script: %s\n", buf);
		break;
	case DHCP_TAG_OPTIONS:
		buf[len] = 0;
		xprintf("BOLT Boot Options: %s\n", buf);
		break;
	default:
...
{
	if (reply->dr_hostname)
		KFREE(reply->dr_hostname);
	if (reply->dr_domainname)
		KFREE(reply->dr_domainname);
	if (reply->dr_bootfile)
		KFREE(reply->dr_bootfile);
	if (reply->dr_rootpath)
		KFREE(reply->dr_rootpath);
	if (reply->dr_swapserver)
		KFREE(reply->dr_swapserver);
	if (reply->dr_script)
		KFREE(reply->dr_script);
	if (reply->dr_options)
		KFREE(reply->dr_options);
	KFREE(reply);
}
...
/**********************************************************************
  ...
  **********************************************************************/
static int dhcp_build_discover(uint8_t *hwaddr, uint32_t id, ebuf_t **ebufptr)
{
...
/**********************************************************************
  ...
  *	 else error
  **********************************************************************/
static int dhcp_wait_reply(int s, uint32_t id, dhcpreply_t *reply,
			   int expfcode)
{
	uint32_t tmpd;
	uint8_t tmpb;
	...
	while (!TIMER_EXPIRED(timer)) {
		...
		if (*hostname)
			reply->dr_hostname = strdup(hostname);
		if (*bootfile)
			reply->dr_bootfile = strdup(bootfile);
		...
		memcpy(reply->dr_dhcpserver_hwaddr, ebuf_srcaddr(buf), ENET_ADDR_LEN);
		...
		if (tmpd == DHCP_MAGIC_NUMBER) {
			...
			while (buf->eb_length > 0) {
				...
				case DHCP_TAG_FUNCTION:
					...
					break;
				case DHCP_TAG_NETMASK:
					memcpy(reply->dr_netmask, junk,
					       IP_ADDR_LEN);
					break;
				case DHCP_TAG_GATEWAY:
					memcpy(reply->dr_gateway, junk,
					       IP_ADDR_LEN);
					break;
				case DHCP_TAG_NAMESERVER:
					memcpy(reply->dr_nameserver, junk,
					       IP_ADDR_LEN);
					break;
				case DHCP_TAG_DOMAINNAME:
					junk[len] = 0;
