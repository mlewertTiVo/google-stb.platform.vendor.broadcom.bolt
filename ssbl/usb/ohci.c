...
/* indicate items that may need endian swapping (e.g. if big endian on PCI)
*/
#define BSWAP32(x) (x)
...
{
	printf("OHCI Port %d: %08X  ", idx, reg);
...
void ohci_dumptd(ohci_td_t *td)
{
	...
	printf
	    ("OHCI [%08X] ctl=%08X (DP=%s,DI=%d,T=%d,EC=%d,CC=%d%s) cbp=%08X be=%08X next=%08X\n",
	     OHCI_VTOP(td), ctl, pids[G_OHCI_TD_PID(ctl)], G_OHCI_TD_DI(ctl),
	     G_OHCI_TD_DT(ctl), G_OHCI_TD_EC(ctl), G_OHCI_TD_CC(ctl),
	     (ctl & M_OHCI_TD_SHORTOK) ? ",R" : "", BSWAP32(td->td_cbp),
	     BSWAP32(td->td_be), BSWAP32(td->td_next_td));
}
...
void ohci_dumptdchain(ohci_td_t *td)
{
	...
	for (;;) {
		printf("OHCI %d:[%08X] ctl=%08X cbp=%08X be=%08X next=%08X\n",
		       idx,
		       OHCI_VTOP(td),
...
void ohci_dumped(ohci_ed_t *ed)
{
	...
	ctl = BSWAP32(ed->ed_control),
	    printf
	    ("OHCI [%08X] Ctl=%08X (MPS=%d%s%s%s,EN=%d,FA=%d,D=%s) Tailp=%08X headp=%08X next=%08X %s\n",
	     OHCI_VTOP(ed), ctl, G_OHCI_ED_MPS(ctl),
	     (ctl & M_OHCI_ED_LOWSPEED) ? ",LS" : "",
	     (ctl & M_OHCI_ED_SKIP) ? ",SKIP" : "",
	     (ctl & M_OHCI_ED_ISOCFMT) ? ",ISOC" : "", G_OHCI_ED_EN(ctl),
	     G_OHCI_ED_FA(ctl), pids[G_OHCI_ED_DIR(ctl)], BSWAP32(ed->ed_tailp),
	     BSWAP32(ed->ed_headp), BSWAP32(ed->ed_next_ed),
...
static ohci_transfer_t *_ohci_allocxfer(ohci_softc_t *softc)
{
	...
	if (ohcidebug > 2) {
		...
		printf("OHCI AllocXfer: %d left, %d inuse\n", cnt,
	...
	if (!t) {
		printf("OHCI No more transfer descriptors!\n");
		return NULL;
	}
...
static void _ohci_freeept(ohci_softc_t *softc, ohci_endpoint_t *e)
{
	if (ohcidebug > 2) {
		...
		printf("OHCI FreeEpt[%p]: %d left, %d inuse\n", e, cnt,
...
static int _ohci_initpools(ohci_softc_t *softc)
{
	...
	softc->ohci_transfer_pool =
	    KUMALLOC(OHCI_TDPOOL_SIZE * sizeof(ohci_transfer_t), DMA_BUF_ALIGN);
	softc->ohci_hwtdpool =
	    KUMALLOC(OHCI_TDPOOL_SIZE * sizeof(ohci_td_t), DMA_BUF_ALIGN);
	...
	if (!softc->ohci_transfer_pool || !softc->ohci_hwtdpool) {
		printf("OHCI Could not allocate transfer descriptors\n");
		return -1;
	}
	...
	softc->ohci_endpoint_pool =
	    KUMALLOC(OHCI_EDPOOL_SIZE * sizeof(ohci_endpoint_t), DMA_BUF_ALIGN);

	softc->ohci_hwedpool =
	    KUMALLOC(OHCI_EDPOOL_SIZE * sizeof(ohci_ed_t), DMA_BUF_ALIGN);
	...
	softc->ohci_hcca = KUMALLOC(sizeof(ohci_hcca_t), sizeof(ohci_hcca_t));
...
static int ohci_start(usbbus_t *bus)
{
	...
	OHCI_WRITECSR(softc, R_OHCI_CONTROL,
	...
	for (idx = 0; idx < 10000; idx++) {
		if (!
		    (OHCI_READCSR(softc, R_OHCI_CMDSTATUS) &
	...
	OHCI_WRITECSR(softc, R_OHCI_CONTROLHEADED,
...
static int _ohci_setupepts(ohci_softc_t *softc)
{
	 ...
	 *
	 * For the purposes of BOLT, it's probably not necessary
	...
	for (idx = 0; idx < OHCI_INTTREE_SIZE; idx++) {
		...
		child = (idx ==  0) ? softc->ohci_isoc_list :
					softc->ohci_edtable[(idx - 1) / 2];
...
static void _ohci_queueept(ohci_softc_t *softc, ohci_endpoint_t *queue,
...
static void _ohci_deqept(ohci_softc_t *softc, ohci_endpoint_t *queue,
			 ohci_endpoint_t *e)
{
	ohci_endpoint_t *cur;
	ohci_ed_t *cured;
	...
	if (cur == NULL) {
		printf("OHCI Could not remove EP %08X: not on the list!\n",
		       (uint32_t) (intptr_t) e);
		return;
	}
...
static void ohci_intr_procdoneq(ohci_softc_t *softc)
{
	...
	while (doneq) {
		...
		if (ohcidebug > 1) {
			ur = transfer->t_ref;
			printf("OHCI Done(%d): ", ur ? ur->ur_tdcount : -1);
			ohci_dumptd(td);
		}
		...
		if (val != 0) {
			...
			if (ed->ed_control & M_OHCI_ED_SKIP) {
				val = K_OHCI_CC_CANCELLED;
			} else if ((val != 4) || /* show stalls only on EP0 */
				(G_OHCI_ED_EN(ed->ed_control) == 0)) {
				printf("OHCI [Transfer error: %d]\n", val);
				val = BOLT_ERR_IOERR;
			}
		}
		...
		if (ur) {
			...
			if (BSWAP32(td->td_cbp) == 0) {
				...
				ur->ur_xferred += transfer->t_length -
				    (BSWAP32(td->td_be) - BSWAP32(td->td_cbp) +
				     1);
			}
			if (ur->ur_tdcount == 0) {
...
static int ohci_intr(usbbus_t *bus)
{
	...
	if (reg & M_OHCI_INT_SO) {
		printf("OHCI SchedOverrun\n");
		OHCI_WRITECSR(softc, R_OHCI_CONTROL,
			      (OHCI_READCSR(softc, R_OHCI_CONTROL) &
			       ~M_OHCI_CONTROL_PLE));
	}
	...
	if (reg & M_OHCI_INT_RD)
		printf("OHCI ResumeDetect\n");

	/* Unrecoverable errors */
	if (reg & M_OHCI_INT_UE)
		printf("OHCI UnrecoverableError\n");
	...
	if ((reg & ~softc->ohci_intdisable) & M_OHCI_INT_RHSC) {
		...
		if (ohcidebug > 0) {
			printf("OHCI RootHubStatusChange: ");
			lreg = OHCI_READCSR(softc, R_OHCI_RHSTATUS);
			ohci_dumprhstat(lreg);
			lreg = OHCI_READCSR(softc, R_OHCI_RHPORTSTATUS(1));
			ohci_dumpportstat(1, lreg);
		}
...
static usbbus_t *ohci_create(physaddr_t addr)
{
	...
	softc = KUMALLOC(sizeof(ohci_softc_t), DMA_BUF_ALIGN);
	if (!softc)
		return NULL;
				  ...
				  int usbaddr, int eptnum, int mps, int flags)
{
	...
	if (ohcidebug > 0) {
		printf("OHCI Create endpoint %d addr %d flags %08X mps %d\n",
...
static void ohci_ept_delete(usbbus_t *bus, usb_ept_t *uept)
{
	...
	while ((OHCI_READCSR(softc, R_OHCI_FMNUMBER) & 0xFFFF) == framenum)
		; /* NULL LOOP */
	...
	while (tdphys != BSWAP32(ed->ed_tailp)) {
		...
		if (ur) {
			...
			if (ur->ur_tdcount == 0) {
				if (ohcidebug > 0) {
					printf("OHCI Completing request due to: ");
					printf("closed pipe %p\n", ur);
				}
				usb_complete_request(ur, K_OHCI_CC_CANCELLED);
				/* XXX it is expected that the
				callee will free the usbreq. */
			}
		}
...
/*	pktlen = ept->ep_mps; */
	...
	if (ur->ur_flags & UR_FLAG_SETUP) {
		tdcontrol = V_OHCI_TD_PID(K_OHCI_TD_SETUP) |
		    ...
		    V_OHCI_TD_CC(K_OHCI_CC_NOTACCESSED) | V_OHCI_TD_DI(0);
	...
	if (ohcidebug > 1) {
		printf
		    (">> OHCI Queueing xfer addr %d pipe %d ED %08X ptr %016llX length %d\n",
		     ur->ur_dev->ud_address, ur->ur_pipe->up_num, ept->ep_phys,
		     (uint64_t) (uintptr_t) ptr, len);
	/* ohci_dumped(ed); */
	}
	...
	/* SK...fix to get around hang when device communication fails */
	ed->ed_headp &= BSWAP32(~1);	/* clear halted state */
...
static int ohci_roothub_req(ohci_softc_t *softc, usb_device_request_t *req)
{
	...
	case REQCODE(USB_REQUEST_CLEAR_FEATURE,
		...
		if (ohcidebug > 0) {
			printf("OHCI RHClearFeature(%d): ", wValue);
			ohci_dumpportstat(wIndex,
					  OHCI_READCSR(softc, statport));
		}
		switch (wValue) {
...
static int ohci_roothub_xfer(usbbus_t *bus, usb_ept_t *uept, usbreq_t *ur)
{
	...
	case 0:
		...
		if (ur->ur_flags & UR_FLAG_SETUP) {
			...
			if (res != 0)
				printf("OHCI Root hub request returned an error\n");
			...
			printf("OHCI Unknown root hub transfer type\n");
			return -1;
		}
		break;
