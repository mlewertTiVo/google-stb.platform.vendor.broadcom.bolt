...
static void usbhid_kbd_mod1(usbhid_softc_t *uhid)
{
	...
	changed =
	    (uhid->uhid_imsg[UBR_KBD_MODS] ^ uhid->uhid_lastmsg[UBR_KBD_MODS]);
	if (changed == 0)
		return;
	...
	if (mod & (KBD_MOD_LCTRL | KBD_MOD_RCTRL))
		uhid->uhid_shiftflags |= FLG_CTRL;
	if (mod & (KBD_MOD_LSHIFT | KBD_MOD_RSHIFT))
		uhid->uhid_shiftflags |= FLG_SHIFT;
	if (mod & (KBD_MOD_LALT | KBD_MOD_RALT))
		uhid->uhid_shiftflags |= FLG_ALT;
}
...
static void usbhid_kbd_scan1(usbhid_softc_t *uhid, uint8_t scan, int breakflg)
{
	...
	if (code->kc_type & (FLG_CAPS | FLG_SCROLL | FLG_NUM)) {
		...
		if (!breakflg)
			uhid->uhid_shiftflags ^= code->kc_type;

/*		if (ks->ks_setleds) {
			(*(ks->ks_setleds))(ks,ks->ks_shiftflags
				& (FLG_CAPS|FLG_SCROLL|FLG_NUM));
		}
*/
	}
	...
	if (code->kc_type & (FLG_ASCII | FLG_FKEY | FLG_NKPD)) {
		...
		if (uhid->uhid_shiftflags & (FLG_SHIFT | FLG_CAPS))
			str = code->kc_shifted;
		else if (uhid->uhid_shiftflags & FLG_CTRL)
			str = code->kc_ctrl;
		else
			str = code->kc_normal;
...
static void usbhid_kbd_scan(usbhid_softc_t *uhid)
{
	...
	for (n = UBR_KBD_KEYS;
		n < (UBR_KBD_KEYS + UBR_KBD_NUMKEYS); n++) {

		if (uhid->uhid_imsg[n] == 0)
			break;	/* no more keys */

		for (o = UBR_KBD_KEYS;
			o < (UBR_KBD_KEYS + UBR_KBD_NUMKEYS); o++) {
			if (uhid->uhid_imsg[n] == uhid->uhid_lastmsg[o])
				break;
		}

		if (o == (UBR_KBD_KEYS + UBR_KBD_NUMKEYS)) {
			/* key not found, must be pressed */
	...
	for (n = UBR_KBD_KEYS;
		...
		if (uhid->uhid_lastmsg[n] == 0)
			break;	/* no more keys */

		for (o = UBR_KBD_KEYS;
			o < (UBR_KBD_KEYS + UBR_KBD_NUMKEYS); o++) {
			if (uhid->uhid_lastmsg[n] == uhid->uhid_imsg[o])
				break;
		}

		if (o == (UBR_KBD_KEYS + UBR_KBD_NUMKEYS)) {
			/* key not found, must be released */
...
#if 1	/* ifdef CFG_TEST_MOUSE */
		if (env_getenv("USBMOUSETEST")) {
			/*
...
static int usbhid_attach(usbdev_t *dev, usb_driver_t *drv)
{
	...
	softc->uhid_imsg = KMALLOC(UBR_KBD_MAX, DMA_BUF_ALIGN);
...
static int usbhid_detach(usbdev_t *dev)
{
	...
	softc = (usbhid_softc_t *) dev->ud_private;

	if (softc) {
		KFREE(softc->uhid_imsg);
		KFREE(softc);
	}
