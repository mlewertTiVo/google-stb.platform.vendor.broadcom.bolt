...
int usb_create_pipe(usbdev_t *dev, int epaddr, int mps, int flags)
{
	...
	pipe = KMALLOC(sizeof(usbpipe_t), DMA_BUF_ALIGN);
...
usbdev_t *usb_create_device(usbbus_t *bus, int speed)
{
	...
	dev = KMALLOC(sizeof(usbdev_t), DMA_BUF_ALIGN);
	...
	if (speed == LS) {
		...
		pipeflags |= UP_TYPE_HIGHSPEED;
		dev->ud_flags |= UD_FLAG_HIGHSPEED;
	}
...
usbreq_t *usb_make_request(usbdev_t *dev, int epaddr, uint8_t *buf,
			   int length, int flags)
{
	...
	ur = KMALLOC(sizeof(usbreq_t), DMA_BUF_ALIGN);
...
int usb_queue_request(usbreq_t *ur)
{
	...
	if (ur->ur_dev->ud_flags & UD_FLAG_ERROR)
		res = BOLT_ERR_IOERR;
	else {
		ur->ur_inprogress = 1;
		ur->ur_xferred = 0;
...
{
	int to = 500000;	/* ~5 seconds at 10 uS per loop */

	if (ur->ur_dev->ud_flags & UD_FLAG_ERROR)
		ur->ur_status = BOLT_ERR_IOERR;
	else {
		       ...
		       int wValue, int wIndex)
{
	...
	requestbuf = KMALLOC(32, DMA_BUF_ALIGN);
	...
	if (usb_noisy > 1)
		printf("Request: ");
	ur = usb_make_request(dev, 0, requestbuf, sizeof(usb_device_request_t),
			      UR_FLAG_SETUP);
...
int usb_new_address(usbbus_t *bus)
{
	...
	printf("Exceeded max devices on BUS\n");
	return -1;
}
...
int usb_set_address(usbdev_t *dev, int address)
{
	...
	if (res == 0) {
		/*
		 * Section 9.2.6.3 allows a device to have 2ms as recovery 
		 * interval, but Linux uses 10ms for 'quirky hardware' to be
		 * safer. Use the same timeout.
		 */
		usb_delay_ms(dev->ud_bus, 10); /* 10ms delay */
		dev->ud_bus->ub_devices[address] = dev;
		dev->ud_address = address;
		for (idx = 0; idx < UD_MAX_PIPES; idx++) {
			pipe = dev->ud_pipes[idx];
			if (pipe && pipe->up_hwendpoint) {
				UBEPTSETADDR(dev->ud_bus, pipe->up_hwendpoint,
					     address);
			}
		}
	}
		    ...
		    uint16_t wIndex, uint8_t *buffer, int length)
{
	...
	req = KMALLOC(32, DMA_BUF_ALIGN);

	if ((buffer != NULL) && (length != 0)) {

		databuf = KMALLOC(length, DMA_BUF_ALIGN);
	...
	ur = usb_make_request(dev, 0, (uint8_t *) req,
			      sizeof(usb_device_request_t), UR_FLAG_SETUP);
	res = usb_sync_request(ur);
	usb_free_request(ur);
	...
	if ((length != 0) && (databuf != NULL)
	    && (bmRequestType & USBREQ_DIR_IN)) {
		memcpy(buffer, databuf, length);
	}

	if (bmRequestType & USBREQ_DIR_IN) {
		ur = usb_make_request(dev, 0, (uint8_t *) req, 0,
				      UR_FLAG_STATUS_OUT);
	} else {
		ur = usb_make_request(dev, 0, (uint8_t *) req, 0,
				      UR_FLAG_STATUS_IN);
...
int __optimize_O0 usb_get_string(usbdev_t *dev, int id, char *buf, int maxlen)
{
	...
	respbuf = KMALLOC(maxlen * 2 + 2, DMA_BUF_ALIGN);
	...
	amtcopy =
	    usb_get_descriptor(dev, USBREQ_DIR_IN, USB_STRING_DESCRIPTOR_TYPE,
			       id, respbuf, 2);
			      ...
			      int smallflg)
{
	...
	respbuf = KMALLOC(64, DMA_BUF_ALIGN);
	res =
			      ...
			      int idx, int maxlen)
{
	...
	respbuf = KMALLOC(maxlen, DMA_BUF_ALIGN);
...
int usb_get_device_status(usbdev_t *dev, usb_device_status_t *status)
{
	return usb_std_request(dev,
			       ...
			       0,
			       0,
			       (uint8_t *) status, sizeof(usb_device_status_t));
}
